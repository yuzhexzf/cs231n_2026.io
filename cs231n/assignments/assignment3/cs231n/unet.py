...
class Unet(nn.Module):
    def __init__(
        ...
        for ind, (dim_in, dim_out) in enumerate(in_out_ups):
            ...
            up_block = nn.ModuleList([
                Upsample(dim_in, dim_out),                    # 上采样并降通道数到 dim_out
                ResnetBlock(dim_out*2, dim_out, context_dim=context_dim),  # 输入为拼接后的 2*dim_out
    ...
    def cfg_forward(self, x, time, model_kwargs={}):
        ...
        ##################################################################
        # 1. 构建有条件和无条件的模型输入
        cond_kwargs = model_kwargs.copy()                # 保留原始条件（如 text_emb）
        uncond_kwargs = model_kwargs.copy()
        # 无条件时，将 text_emb 设置为 None（模型内部会处理为零向量）
        uncond_kwargs["text_emb"] = None

        # 2. 获取两个预测：有条件预测和无条件预测
        eps_cond = self.forward(x, time, cond_kwargs)      # ϵ_θ(x_t, t, c)
        eps_uncond = self.forward(x, time, uncond_kwargs)  # ϵ_θ(x_t, t, ∅)

        # 3. 应用 CFG 公式：ϵ_guided = (scale + 1) * ϵ_cond - scale * ϵ_uncond
    ...
    def forward(self, x, time, model_kwargs={}):
        ...
        #    - Make sure to pass the context to each ResNet block.
        ##################################################################
        # ---------- Downsampling (store two skip connections per block) ----------
        skips = []  # each element will be a tuple (skip1, skip2)
        for down_block in self.downs:
            # down_block = [ResBlock1, ResBlock2, Downsample]
            x = down_block[0](x, context)          # after first ResBlock
            skip1 = x
            x = down_block[1](x, context)          # after second ResBlock
            skip2 = x
            skips.append((skip1, skip2))
            x = down_block[2](x)                   # Downsample

        # ---------- Middle ----------
        x = self.mid_block1(x, context)
        x = self.mid_block2(x, context)

        # ---------- Upsampling (use saved skips in reverse order) ----------
        # Reverse skips list so that we pop from deepest block first
        skips = skips[::-1]
        for up_block in self.ups:
            # up_block = [Upsample, ResBlock1, ResBlock2]
            x = up_block[0](x)                     # Upsample (also reduces channels)
            # Get the corresponding skip tuple for this level
            skip1, skip2 = skips.pop(0)            # skip1 from first ResBlock, skip2 from second

            # First ResBlock: input = [x, skip2] (concat) -> output channels = 2*dim_out
            x = torch.cat([x, skip2], dim=1)
            x = up_block[1](x, context)            # ResBlock1 (2*dim_out → 2*dim_out)

            # Second ResBlock: input = [x, skip1] (concat) -> output channels = 2*dim_out? 注意这里原注释要求第二个 ResBlock 输入也是 2*dim_out
            x = torch.cat([x, skip1], dim=1)
            x = up_block[2](x, context)            # ResBlock2 (2*dim_out → dim_out)
        ##################################################################
