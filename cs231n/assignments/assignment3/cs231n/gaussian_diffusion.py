...
class GaussianDiffusion(nn.Module):
    ...
    def p_sample(self, x_t, t: int, model_kwargs={}):
        ...
        """
        t_tensor = torch.full((x_t.shape[0],), t, device=x_t.device, dtype=torch.long)  # (b,)
        ...
        pred = self.model(x_t, t_tensor, model_kwargs)

        # 2. Convert prediction to x_start and noise as needed
        if self.objective == "pred_noise":
            noise = pred
            x_start = self.predict_start_from_noise(x_t, t_tensor, noise)
        else:  # pred_x_start
            x_start = pred
            noise = self.predict_noise_from_start(x_t, t_tensor, x_start)

        # 3. Clamp x_start to valid range [-1, 1] for stability
        x_start = x_start.clamp(-1., 1.)

        # 4. Get posterior mean and std for q(x_{t-1} | x_t, x_0)
        #    Use precomputed posterior coefficients (posterior_mean_coef1, coef2, posterior_std)
        #    Alternatively, a helper method self.q_posterior can be defined.
        #    Here we directly compute using the buffers.
        coef1 = extract(self.posterior_mean_coef1, t_tensor, x_t.shape)
        coef2 = extract(self.posterior_mean_coef2, t_tensor, x_t.shape)
        posterior_std = extract(self.posterior_std, t_tensor, x_t.shape)

        # Posterior mean = coef1 * x_start + coef2 * x_t
        posterior_mean = coef1 * x_start + coef2 * x_t

        # Sample x_{t-1} ~ N(posterior_mean, posterior_std^2)
        # For t=0 (final step), add no noise; for t>0, add noise
        if t == 0:
