...
def sim(z_i, z_j):
    ...
    ##############################################################################
    # Compute dot product
    dot = torch.dot(z_i, z_j)  # scalar tensor
    # Compute norms
...
def simclr_loss_naive(out_left, out_right, tau):
    ...
    for k in range(N):  # loop through each positive pair (k, k+N)
        ...
        ##############################################################################
        # compute similarity for positive pair
        sim_pos = sim(z_k, z_k_N)  # scalar
        
        # denominator for z_k (anchor)
        denom_k = 0
        for i in range(2*N):
            if i == k:
                continue
            denom_k += torch.exp(sim(z_k, out[i]) / tau)
        l1 = -sim_pos / tau + torch.log(denom_k)
        
        # denominator for z_{k+N} (anchor)
        denom_kN = 0
        for i in range(2*N):
            if i == k+N:
                continue
            denom_kN += torch.exp(sim(z_k_N, out[i]) / tau)
        l2 = -sim_pos / tau + torch.log(denom_kN)
...
def sim_positive_pairs(out_left, out_right):
    ...
    # HINT: torch.linalg.norm might be helpful.                                  #
    ##############################################################################
    # Compute dot product per row (element-wise multiply then sum over dimension 1)
    dot = torch.sum(out_left * out_right, dim=1, keepdim=True)  # N x 1
    
    # Compute L2 norm per row (keep dimension for broadcasting)
    norm_left = torch.linalg.norm(out_left, dim=1, keepdim=True)  # N x 1
    norm_right = torch.linalg.norm(out_right, dim=1, keepdim=True)  # N x 1
    
    # Normalized dot product (cosine similarity)
    pos_pairs = dot / (norm_left * norm_right)
    ##############################################################################
    #                               END OF YOUR CODE                             #
    ##############################################################################
    return pos_pairs
...
def compute_sim_matrix(out):
    ...
    # Compute cosine similarity matrix: out_norm @ out_norm.T
...
def simclr_loss_vectorized(out_left, out_right, tau, device='cuda'):
    ...
    sim_pos = torch.cat([sim_pos_pair, sim_pos_pair], dim=0).squeeze()  # [2N]
    
    # Step 3: Compute the numerator value for all augmented samples.
    numerator = torch.exp(sim_pos / tau)  # [2N]
    
    
    # Step 4: Now that you have the numerator and denominator for all augmented samples, compute the total loss.
    loss_per_anchor = -sim_pos / tau + torch.log(denom.squeeze())  # [2N]
    loss = loss_per_anchor.mean()  # scalar
    ##############################################################################
    #                               END OF YOUR CODE                             #
    ##############################################################################
